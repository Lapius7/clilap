...
def render_not_found(path, nc):
    ...
    return '\n'.join([
        ...
        c(BC, '    /log       /cidr      /asn       /robots'),
        c(BC, '    /useragent /jwt       /morse     /convert'),
        c(BC, '    /moon      /sunrise'),
        '',
...
def render_help(nc):
    ...
    lines = [
        ...
        h('/cidr  🌐 CIDR計算'),
        '',
        sec('概要', 'IPv4/IPv6 CIDR表記からネットワーク情報を計算します。'),
        '',
        ex('curl clilap.org/cidr/192.168.1.0/24'),
        ex('curl clilap.org/cidr/10.0.0.0/8'),
        ex('curl clilap.org/cidr/2001:db8::/32'),
        '',
        note('取得できる情報: ネットワーク/ブロードキャストアドレス・サブネットマスク・ホスト数'),

        h('/asn  🌍 ASN情報'),
        '',
        sec('概要', 'IPアドレスが属するASN(自律システム番号)・組織・国を調べます。'),
        '',
        ex('curl clilap.org/asn/8.8.8.8'),
        ex('curl clilap.org/asn/1.1.1.1'),
        '',
        note('Team Cymru の DNS-based lookup を使用。IPv4のみ対応。'),

        h('/robots  🤖 robots.txt 確認'),
        '',
        sec('概要', 'ドメインのrobots.txtを取得し、User-agent毎のルールとSitemapを整形表示します。'),
        '',
        ex('curl clilap.org/robots/github.com'),

        h('/useragent  🕵 User-Agent 解析'),
        '',
        sec('概要', 'User-Agent文字列からブラウザ・OS・デバイス・ボット判定を行います。'),
        '',
        ex('curl clilap.org/useragent              # 自分のUAを解析'),
        ex('curl -A "Mozilla/5.0 ..." clilap.org/useragent  # 指定UAを解析'),

        h('/jwt  🔐 JWT デコーダー'),
        '',
        sec('概要', 'JWTのヘッダ・ペイロードをデコードします。署名検証は行いません。'),
        '',
        ex('curl clilap.org/jwt/{token}'),
        ex('echo "{token}" | curl -d @- clilap.org/jwt'),
        '',
        note('⚠ 署名検証なし。信頼できないトークンの内容を鵜呑みにしないでください。'),

        h('/morse  📡 モールス信号変換'),
        '',
        sec('概要', 'テキストとモールス信号を相互変換します。'),
        '',
        ex('curl clilap.org/morse/encode/SOS'),
        ex('curl clilap.org/morse/decode/..._---_...'),
        ex('echo "Hello" | curl -d @- clilap.org/morse/encode'),
        '',
        note('decode時、文字間は _ (アンダースコア) で区切ってください。単語間は / です。'),

        h('/convert  🔄 フォーマット変換'),
        '',
        sec('概要', 'JSON・YAML・TOML・INI・CSV を相互変換します。'),
        '',
        ex('curl --data-binary @data.json clilap.org/convert/json/yaml'),
        ex('curl --data-binary @data.yaml clilap.org/convert/yaml/json'),
        ex('curl --data-binary @data.csv  clilap.org/convert/csv/json'),
        ex('curl --data-binary @data.toml clilap.org/convert/toml/json'),
        ex('curl --data-binary @data.ini  clilap.org/convert/ini/json'),
        '',
        note('注: -d は改行/インデントを壊すため --data-binary を使用してください'),
        note('注: YAML対応は簡易サブセット (ネスト/リスト/スカラー値のみ)'),

        h('/moon  🌙 月相'),
        '',
        sec('概要', '指定日の月相・月齢・満月までの日数を計算します。'),
        '',
        ex('curl clilap.org/moon              # 今日の月相'),
        ex('curl clilap.org/moon/2026-01-01   # 指定日の月相'),

        h('/sunrise  🌅 日の出・日の入り'),
        '',
        sec('概要', '都市の日の出・日の入り時刻と昼の長さを4日分表示します。'),
        '',
        ex('curl clilap.org/sunrise/Tokyo'),
        ex('curl clilap.org/sunrise          # IPアドレスから自動判定'),
